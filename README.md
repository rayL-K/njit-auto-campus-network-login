...
当前版本的主流程已经改为 **浏览器登录优先**，会优先尝试真实浏览器窗口交互，必要时再退回无界面浏览器方案。
...
- 通过 `drcom/chkstatus` 查询当前在线状态、当前账号和 IP 信息。
- 如果门户显示已在线但账号不是目标账号，或门户在线但外网探测不通，会先尝试注销当前会话并刷新 Wi-Fi，再改用浏览器重新认证。
- 浏览器登录优先使用真实浏览器窗口：账号密码通过 DOM 填值，提交前重新定位登录按钮并优先 DOM 点击，失败时再回退到真实鼠标点击。
- 真实浏览器路径失败时，会自动切换到无界面浏览器模式继续登录。
...
3. 如果已经是目标账号在线，再继续做外网探测；只有外网也恢复，才会直接判定成功。
4. 如果门户显示在线但账号不对，或门户在线但外网探测不通，会先注销当前会话并刷新 Wi-Fi。
5. 进入浏览器登录流程后，优先尝试真实浏览器窗口：先填写账号密码，再重新定位登录按钮并提交。
6. 如果真实浏览器路径失败，则自动切换到无界面浏览器模式继续认证。
7. 登录后再次校验在线状态和账号是否正确。
8. 在等待窗口内重试确认外网连通性；如果仍未恢复，会主动刷新 Wi-Fi 并继续重试，直到超时。
9. 在脚本结束时统一弹出结果通知，并在成功时按需维护本地 ChromeDriver。
